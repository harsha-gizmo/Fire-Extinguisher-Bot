...
def run_simulation_ucs(q, possible_cases = False):
    ...
    while current_pos != button_location:
        ...
        # Backtracking: Check if next position is safe
        temp_grid, temp_fire_cells = spread_fire(grid, fire_cells, q)
        if next_pos in temp_fire_cells:
            # Stay in place if next position is dangerous
...
def main():
    ...
    for q in Q_VALUES:
        ...
        with ProcessPoolExecutor(max_workers=max_cores) as executor:
            ...
            futures = [executor.submit(run_simulation_bfs, q, possible_cases = True) for _ in range(SIMULATION_COUNT)]
    ...
    for q in Q_VALUES:
        ...
        with ProcessPoolExecutor(max_workers=max_cores) as executor:
            # Submit all the BFS simulation tasks to the thread pool
            futures = [executor.submit(run_simulation_ucs, q, possible_cases = True) for _ in range(SIMULATION_COUNT)]
    ...
    for q in Q_VALUES:
        ...
        with ProcessPoolExecutor(max_workers=max_cores) as executor:
            # Submit all the BFS simulation tasks to the thread pool
            futures = [executor.submit(run_simulation_astar, q, possible_cases = True) for _ in range(SIMULATION_COUNT)]
    ...
    for q in Q_VALUES:
        ...
        with ProcessPoolExecutor(max_workers=max_cores) as executor:
            # Submit all the BFS simulation tasks to the thread pool
            futures = [executor.submit(run_simulation_bot4, q, possible_cases = True) for _ in range(SIMULATION_COUNT)]
