...
max_cores = 20
CLUSTER_SIZE = 5  # For creating smaller open clusters
CENTER_SAFE_ZONE_SIZE = 5  # Size of the central safe zone
WALL_PROBABILITY = 0.4  # Reduced probability of placing walls
...
def create_grid_with_walls(size):
    grid = [['blocked' for _ in range(size)] for _ in range(size)]
    ...
    # Create L-shaped blocks
    for i in range(2, size - 2, 6):
        for j in range(2, size - 2, 6):
            # L shape in top-left corner
            for di in range(3):  # Vertical part
                if 0 <= i + di < size:
                    grid[i + di][j] = 'open'
            for dj in range(3):  # Horizontal part
                if 0 <= i + 2 < size and 0 <= j + dj < size:
                    grid[i + 2][j + dj] = 'open'

    return grid

def add_random_openings(grid, num_openings):
    size = len(grid)
...
# Wrapper function for creating Bonus grid layout
def create_space_vessel_layout():
    grid = create_grid_with_walls(GRID_SIZE)
   
    # Add some random openings to make the layout more interesting
    add_random_openings(grid, GRID_SIZE * 2)
   
    # Ensure we have at least TARGET_OPEN_PERCENTAGE of open cells
    while True:
        open_count = sum(row.count('open') for row in grid)
        total_cells = GRID_SIZE * GRID_SIZE
        if open_count >= TARGET_OPEN_PERCENTAGE * total_cells:
            break
        add_random_openings(grid, 10)
